...
class KGAT(object):
    ...
    def _build_model_phase_I(self):
        ...
        #transpose_a=False:在a（u_e）运算前不进行转置
        #transpose_b=True：在b（pos_i_e）运算前进行转置
    ...
    def _create_bi_interaction_embed(self):
        A = self.A_in
        # print("A.shape="+str(A.shape)) #(129955, 129955)
        # print("type(A)="+str(type(A))) #<class 'scipy.sparse.csr.csr_matrix'>
        # print("A[0]="+str(A[0]))#第0行所有有数据的值(0, 23566)       0.008849557522123894 。。。(0, 23678)    0.008849557522123894
        # Generate a set of adjacency sub-matrix.
        A_fold_hat = self._split_A_hat(A)
        # print("len(A_fold_hat)=" + str(len(A_fold_hat)))#100
        # print("type(A_fold_hat)=" + str(type(A_fold_hat)))#<class 'list'>
        # # str(A_fold_hat[0]=SparseTensor(indices=Tensor("SparseTensor/indices:0", shape=(154915, 2), dtype=int64),
        # # values=Tensor("SparseTensor/values:0", shape=(154915,), dtype=float32),
        # # dense_shape=Tensor("SparseTensor/dense_shape:0", shape=(2,), dtype=int64))
        # print("str(A_fold_hat[0]=" + str(A_fold_hat[0]))
        # #SparseTensor(indices=Tensor("SparseTensor_1/indices:0", shape=(116530, 2), dtype=int64),
        # # values=Tensor("SparseTensor_1/values:0", shape=(116530,), dtype=float32),
        # # dense_shape=Tensor("SparseTensor_1/dense_shape:0", shape=(2,), dtype=int64))
        # print("str(A_fold_hat[1]=" + str(A_fold_hat[1]))
        #e_{N_h}
        # print("self.weights['user_embed'].shape="+str(self.weights['user_embed'].shape))#(23566, 64)
        # print("self.weights['entity_embed'].shape="+str(self.weights['entity_embed'].shape))#(106389, 64)
        ego_embeddings = tf.concat([self.weights['user_embed'], self.weights['entity_embed']], axis=0)
        # print("ego_embeddings.shape="+str(ego_embeddings.shape))#(129955, 64)
        all_embeddings = [ego_embeddings]
        # print("all_embeddings.len="+str(len(all_embeddings)))#1
