...
class Data(object):
    ...
    def _statistic_ratings(self):
        self.n_users = max(max(self.train_data[:, 0]), max(self.test_data[:, 0])) + 1#23566
        self.n_items = max(max(self.train_data[:, 1]), max(self.test_data[:, 1])) + 1#48123
        self.n_train = len(self.train_data)#1289003
        self.n_test = len(self.test_data)#423635
    ...
    def _load_kg(self, file_name):
        ...
        #去重，axis=0说明要整行完全相同才去除
        kg_np = np.unique(kg_np, axis=0)

        # self.n_relations = len(set(kg_np[:, 1]))
        # self.n_entities = len(set(kg_np[:, 0]) | set(kg_np[:, 2]))
        self.n_relations = max(kg_np[:, 1]) + 1#9
        self.n_entities = max(max(kg_np[:, 0]), max(kg_np[:, 2])) + 1#106389
        self.n_triples = len(kg_np)#464567
